...
app.get("/", (c) => {
  return c.json({
    name: "legal-bot-server",
    description: "Legal Service — Claude Code REST API Server (Agent SDK)",
    version: "1.0.0",
    endpoints: [
...
app.get("/health", (c) => {
  return c.json({ status: "ok", service: "legal-bot-server", timestamp: new Date().toISOString() })
})
...
console.log("Legal Bot Server v1.0 (Claude Agent SDK)")
